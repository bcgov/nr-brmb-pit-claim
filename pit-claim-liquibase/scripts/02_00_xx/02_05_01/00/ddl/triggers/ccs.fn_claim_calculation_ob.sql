...
BEGIN
    ...
    IF (TG_OP = 'UPDATE' AND NEW.insurance_plan_id = 3 AND NEW.commodity_coverage_code = 'CQNT' AND NEW.calculation_status_code <> OLD.calculation_status_code ) THEN
		...
		VALUES ( nextval('ccs.ccb_ob_seq'), 
				(select max(claim_calculation_berries_guid)
				from claim_calculation_berries
				where claim_calculation_guid = NEW.claim_calculation_guid), 
				TG_OP, 
				NEW.create_user, 
				NEW.create_date, 
				NEW.update_user, 
...
AFTER UPDATE ON ccs.claim_calculation
FOR EACH ROW
